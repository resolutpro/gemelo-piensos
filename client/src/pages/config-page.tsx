...
export default function ConfigPage() {
  ...
  //Estados para saber qué elemento se está editando
  ...
  // NUEVO: Funciones para abrir los modales en modo EDICIÓN
  ...
  const openEditMaterial = (material: RawMaterial) => {
    ...
    // Nota: Ajusta los campos según los que guardes realmente en BD
    materialForm.reset({
      name: material.name,
      code: material.code,
      unit: material.unit || "kg",
      category: "cereal" as any,
  ...
  // NUEVO: Mutaciones para actualizar (PATCH)
  const updateZone = useMutation({
    mutationFn: async (data: any) => {
      const r = await apiRequest("PATCH", `/api/zones/${data.id}`, data);
      return r.json();
    },
    onSuccess: () => {
  ...
  const updateSupplier = useMutation({
    mutationFn: async (data: any) => {
      const r = await apiRequest("PATCH", `/api/suppliers/${data.id}`, data);
      return r.json();
    },
    onSuccess: () => {
  ...
  const createZone = useMutation({
    mutationFn: async (data: ZoneFormData) => {
      const r = await apiRequest("POST", "/api/zones", data);
      return r.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/zones"] });
  ...
  const createMaterial = useMutation({
    mutationFn: async (data: MaterialFormData) => {
      const r = await apiRequest("POST", "/api/raw-materials", data);
      return r.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/raw-materials"] });
      toast({ title: "Materia prima creada" });
      setMaterialOpen(false);
      materialForm.reset();
    },
    onError: (err: any) =>
  ...
  const createSupplier = useMutation({
    mutationFn: async (data: SupplierFormData) => {
      const r = await apiRequest("POST", "/api/suppliers", data);
      return r.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/suppliers"] });
  ...
  const deleteZone = useMutation({
    ...
    onError: (err: any) =>
      toast({
        ...
        description: "La zona puede estar en uso",
        variant: "destructive",
      }),
  });
  ...
  const deleteMaterial = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/raw-materials"] });
      toast({ title: "Materia prima eliminada" });
    },
    onError: (err: any) =>
      toast({
        title: "No se puede eliminar",
        description: "La materia prima puede estar en uso",
        variant: "destructive",
      }),
  });
  ...
  const deleteSupplier = useMutation({
    ...
    onError: (err: any) =>
      toast({
        title: "No se puede eliminar",
        description: "El proveedor puede estar en uso",
        variant: "destructive",
      }),
  });
    ...
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Catálogo de ingredientes para la fábrica
              </p>
            </div>
            <Button
              size="sm"
          ...
          {rawMaterials.length === 0 && (
            <Card>
              <CardContent className="py-12 text-center space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Sin materias primas. Añada las primeras para comenzar.
                </p>
              </CardContent>
            </Card>
          )}
        ...
        <TabsContent value="zones" className="mt-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Divisiones geográficas para sensores y gestión
              </p>
            </div>
            <Button
              size="sm"
            ...
            {zones.length === 0 ? (
              <Card>
                <CardContent className="py-12 text-center space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Sin zonas definidas. Cree zonas para organizar los sensores.
                  </p>
                </CardContent>
              </Card>
            ) : (
              zones.map((z) => (
                <EntityCard
                  key={z.id}
                  name={z.name}
                  code={z.code ?? undefined} // <--- Cambiar z.location por z.code
        ...
        <TabsContent value="suppliers" className="mt-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Empresas suministradoras de materias primas
              </p>
            </div>
            <Button
              size="sm"
            ...
            {suppliers.length === 0 ? (
              <Card>
                <CardContent className="py-12 text-center space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Sin proveedores. Añada proveedores para asociarlos a los
                    lotes.
                  </p>
                </CardContent>
              </Card>
      ...
      <Dialog open={zoneOpen} onOpenChange={setZoneOpen}>
        <DialogContent className="max-w-md">
          ...
          <form
            onSubmit={zoneForm.handleSubmit((d) => createZone.mutate(d))}
      ...
      {/* Dialog para Nueva Materia Prima */}
      <Dialog open={materialOpen} onOpenChange={setMaterialOpen}>
        <DialogContent className="max-w-lg">
          ...
          <form
            onSubmit={materialForm.handleSubmit((d) =>
              createMaterial.mutate(d),
            )}
            className="pt-2"
          >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder="Maíz Grano"
                    data-testid="input-material-name"
                  />
                  {materialForm.formState.errors.name && (
                    <p className="text-xs text-destructive">
                      {materialForm.formState.errors.name.message}
                    </p>
                  )}
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    onValueChange={(v) =>
                      materialForm.setValue("category", v as any)
                    }
                  >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input
                    type="number"
                    step="0.1"
                    {...materialForm.register("targetMoistureMax")}
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input
                    type="number"
                    step="0.1"
                    {...materialForm.register("targetProteinMin")}
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input
                    type="number"
                    step="0.1"
                    {...materialForm.register("targetFatMax")}
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input
                    type="number"
                    step="0.1"
                    {...materialForm.register("targetFiberMax")}
                  />
                </div>
              </div>
              <div className="space-y-1.5">
      ...
      <Dialog open={supplierOpen} onOpenChange={setSupplierOpen}>
        <DialogContent className="max-w-md">
          ...
          <form
            onSubmit={supplierForm.handleSubmit((d) =>
          ...
          >
            <div className="max-h-[60vh] overflow-y-auto pr-2 space-y-4 pb-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  type="email"
                  {...supplierForm.register("contactEmail")}
                />
                {supplierForm.formState.errors.contactEmail && (
                  <p className="text-xs text-destructive">
                    {supplierForm.formState.errors.contactEmail.message}
                  </p>
                )}
              </div>
              <div className="space-y-1.5">

...
export const rawMaterials = pgTable("raw_materials", {
  id: integer("id").primaryKey().generatedAlwaysAsIdentity(),
  name: text("name").notNull(),
  code: text("code").unique(), // Ya está opcional gracias al paso anterior
  category: text("category").notNull().default("other"), // <--- NUEVO CAMPO AÑADIDO
...
export const zones = pgTable("zones", {
  id: integer("id").primaryKey().generatedAlwaysAsIdentity(),
  name: text("name").notNull(),
  code: text("code").unique(), // <--- Le quitamos el .notNull()
  description: text("description"),
  active: boolean("active").notNull().default(true),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const insertRawMaterialLotSchema = createInsertSchema(
  rawMaterialLots,
...
export const insertNirAnalysisSchema = createInsertSchema(nirAnalyses).omit({
  id: true,
  createdAt: true,
});
...
export const insertMixSimulationSchema = createInsertSchema(
  mixSimulations,
).omit({ id: true, createdAt: true });
...
export const insertMixSimulationItemSchema = createInsertSchema(
  mixSimulationItems,
).omit({ id: true, createdAt: true });
export type InsertMixSimulationItem = z.infer<
  typeof insertMixSimulationItemSchema
>;
...
export const insertProductionBatchSchema = createInsertSchema(
  productionBatches,
).omit({ id: true, createdAt: true });
...
export const insertFinalProductAnalysisSchema = createInsertSchema(
  finalProductAnalyses,
).omit({ id: true, createdAt: true });
export type InsertFinalProductAnalysis = z.infer<
  typeof insertFinalProductAnalysisSchema
>;
...
export const rawMaterialLotsRelations = relations(
  rawMaterialLots,
  ({ one, many }) => ({
    rawMaterial: one(rawMaterials, {
      fields: [rawMaterialLots.rawMaterialId],
      references: [rawMaterials.id],
    }),
    supplier: one(suppliers, {
      fields: [rawMaterialLots.supplierId],
...
export const mixSimulationsRelations = relations(
  mixSimulations,
  ({ one, many }) => ({
    recipe: one(recipes, {
      fields: [mixSimulations.recipeId],
...
export const mixSimulationItemsRelations = relations(
  mixSimulationItems,
  ({ one }) => ({
    simulation: one(mixSimulations, {
      fields: [mixSimulationItems.simulationId],
      references: [mixSimulations.id],
    }),
    lot: one(rawMaterialLots, {
      fields: [mixSimulationItems.lotId],
      references: [rawMaterialLots.id],
    }),
  }),
);
...
export const productionBatchesRelations = relations(
  ...
  ({ one, many }) => ({
    simulation: one(mixSimulations, {
      fields: [productionBatches.simulationId],
      references: [mixSimulations.id],
    }),
...
export const finalProductAnalysesRelations = relations(
  finalProductAnalyses,
  ({ one }) => ({
    batch: one(productionBatches, {
      fields: [finalProductAnalyses.batchId],
      references: [productionBatches.id],
    }),
  }),
);
...
export const traceEventsRelations = relations(traceEvents, ({ one }) => ({
  lot: one(rawMaterialLots, {
    fields: [traceEvents.lotId],
    references: [rawMaterialLots.id],
  }),
  simulation: one(mixSimulations, {
    fields: [traceEvents.simulationId],
    references: [mixSimulations.id],
  }),
  batch: one(productionBatches, {
    fields: [traceEvents.batchId],
    references: [productionBatches.id],
  }),
}));

...
import { z } from "zod";
import {
  insertSupplierSchema, insertRawMaterialSchema, insertZoneSchema,
  insertRawMaterialLotSchema, insertNirAnalysisSchema, insertSensorSchema,
  insertSensorReadingSchema, insertRecipeSchema, insertMixSimulationSchema,
  insertMixSimulationItemSchema, insertProductionBatchSchema,
  insertFinalProductAnalysisSchema, insertAlertSchema, insertTraceEventSchema,
} from "@shared/schema";
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.get("/api/suppliers", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getSuppliers()); } catch (err) { next(err); }
  });
  ...
  app.get("/api/raw-materials", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getRawMaterials()); } catch (err) { next(err); }
  });
  ...
  app.get("/api/zones", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getZones()); } catch (err) { next(err); }
  });
  ...
  app.get("/api/lots", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getLots()); } catch (err) { next(err); }
  });
  ...
  app.patch("/api/lots/:id", requireAuth, async (req, res, next) => {
    try {
      res.json(await storage.updateLot(Number(req.params.id), req.body));
    } catch (err) { next(err); }
  });
  ...
  app.get("/api/nir-analyses", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getNirAnalyses()); } catch (err) { next(err); }
  });

  app.get("/api/nir-analyses/lot/:lotId", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getNirAnalysesByLot(Number(req.params.lotId))); } catch (err) { next(err); }
  });
  ...
  app.post("/api/nir-analyses", requireAuth, async (req, res, next) => {
    try {
      const body = {
        ...req.body,
        analyzedAt: req.body.analyzedAt ? new Date(req.body.analyzedAt) : new Date(),
        lotId: req.body.lotId ? parseInt(req.body.lotId) : null,
        productionBatchId: req.body.productionBatchId ? parseInt(req.body.productionBatchId) : null,
  ...
  app.get("/api/sensors", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getSensors()); } catch (err) { next(err); }
  });
  ...
  app.post("/api/sensors", requireAuth, async (req, res, next) => {
    try {
      const body = {
        ...
        alertThresholdMin: req.body.alertThresholdMin ? parseFloat(req.body.alertThresholdMin) : null,
        alertThresholdMax: req.body.alertThresholdMax ? parseFloat(req.body.alertThresholdMax) : null,
        warningThresholdMin: req.body.warningThresholdMin ? parseFloat(req.body.warningThresholdMin) : null,
        warningThresholdMax: req.body.warningThresholdMax ? parseFloat(req.body.warningThresholdMax) : null,
      };
  ...
  app.post("/api/sensors/:id/readings", requireAuth, async (req, res, next) => {
    try {
      ...
      if (sensor) {
        const exceedsMax = sensor.alertThresholdMax !== null && reading.value > sensor.alertThresholdMax;
        const belowMin = sensor.alertThresholdMin !== null && reading.value < sensor.alertThresholdMin;
        if (exceedsMax || belowMin) {
          ...
          const exceedsWarningMax = sensor.warningThresholdMax !== null && reading.value > sensor.warningThresholdMax;
          const belowWarningMin = sensor.warningThresholdMin !== null && reading.value < sensor.warningThresholdMin;
  ...
  app.post("/api/iot/readings", async (req, res, next) => {
    try {
      ...
      if (!sensorCode || value === undefined) return res.status(400).json({ message: "sensorCode and value required" });
  ...
  app.get("/api/recipes", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getRecipes()); } catch (err) { next(err); }
  });
  ...
  app.get("/api/simulations", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getSimulations()); } catch (err) { next(err); }
  });
  ...
  app.patch("/api/simulations/:id", requireAuth, async (req, res, next) => {
    try {
      const body = {
        ...req.body,
        totalQuantity: req.body.totalQuantity ? parseFloat(req.body.totalQuantity) : undefined,
        estimatedMoisture: req.body.estimatedMoisture !== undefined ? parseFloat(req.body.estimatedMoisture) : undefined,
        estimatedProtein: req.body.estimatedProtein !== undefined ? parseFloat(req.body.estimatedProtein) : undefined,
        estimatedFat: req.body.estimatedFat !== undefined ? parseFloat(req.body.estimatedFat) : undefined,
        estimatedStarch: req.body.estimatedStarch !== undefined ? parseFloat(req.body.estimatedStarch) : undefined,
        estimatedFiber: req.body.estimatedFiber !== undefined ? parseFloat(req.body.estimatedFiber) : undefined,
        estimatedAsh: req.body.estimatedAsh !== undefined ? parseFloat(req.body.estimatedAsh) : undefined,
      };
  ...
  app.get("/api/simulations/:id/items", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getSimulationItems(Number(req.params.id))); } catch (err) { next(err); }
  });

  app.post("/api/simulations/:id/items", requireAuth, async (req, res, next) => {
    try {
      const body = {
  ...
  app.delete("/api/simulation-items/:id", requireAuth, async (req, res, next) => {
    try {
  ...
  app.get("/api/production-batches", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getProductionBatches()); } catch (err) { next(err); }
  });

  app.get("/api/production-batches/:id", requireAuth, async (req, res, next) => {
    try {
  ...
  app.get("/api/final-analyses", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getFinalAnalyses()); } catch (err) { next(err); }
  });

  app.get("/api/final-analyses/batch/:batchId", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getFinalAnalysisByBatch(Number(req.params.batchId))); } catch (err) { next(err); }
  });

  app.post("/api/final-analyses", requireAuth, async (req, res, next) => {
    try {
      const batch = await storage.getProductionBatch(parseInt(req.body.batchId));
      if (!batch) return res.status(404).json({ message: "Lote de producción no encontrado" });
      ...
      const body = {
        ...
        fiber,
        ash,
        moistureDeviation: moisture !== null && simulation.estimatedMoisture ? moisture - simulation.estimatedMoisture : null,
        proteinDeviation: protein !== null && simulation.estimatedProtein ? protein - simulation.estimatedProtein : null,
        fatDeviation: fat !== null && simulation.estimatedFat ? fat - simulation.estimatedFat : null,
        starchDeviation: starch !== null && simulation.estimatedStarch ? starch - simulation.estimatedStarch : null,
        fiberDeviation: fiber !== null && simulation.estimatedFiber ? fiber - simulation.estimatedFiber : null,
        ashDeviation: ash !== null && simulation.estimatedAsh ? ash - simulation.estimatedAsh : null,
  ...
  app.get("/api/recommendations", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getRecommendations()); } catch (err) { next(err); }
  });

  app.get("/api/recommendations/batch/:batchId", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getRecommendationsByBatch(Number(req.params.batchId))); } catch (err) { next(err); }
  });

  app.patch("/api/recommendations/:id", requireAuth, async (req, res, next) => {
    try {
      res.json(await storage.updateRecommendation(Number(req.params.id), req.body));
    } catch (err) { next(err); }
  });
  ...
  app.get("/api/alerts", requireAuth, async (req, res, next) => {
    try { res.json(await storage.getAlerts()); } catch (err) { next(err); }
  });
  ...
  app.get("/api/trace-events", requireAuth, async (req, res, next) => {
    try {
      ...
      if (req.query.simulationId) filters.simulationId = Number(req.query.simulationId);
      if (req.query.batchId) filters.batchId = Number(req.query.batchId);
      res.json(await storage.getTraceEvents(Object.keys(filters).length ? filters : undefined));
    } catch (err) { next(err); }
  });
...
async function recalculateSimulation(simulationId: number) {
  ...
  for (const item of items) {
    ...
    if (nirs && nirs.length > 0) {
      ...
      if (nir.moisture !== null) { wMoisture += nir.moisture * weight; hasNir = true; }
      if (nir.protein !== null) { wProtein += nir.protein * weight; hasNir = true; }
      if (nir.fat !== null) { wFat += nir.fat * weight; hasNir = true; }
      if (nir.starch !== null) { wStarch += nir.starch * weight; hasNir = true; }
      if (nir.fiber !== null) { wFiber += nir.fiber * weight; hasNir = true; }
      if (nir.ash !== null) { wAsh += nir.ash * weight; hasNir = true; }
    }
  }
...
async function generateRecommendations(batchId: number, simulation: any, analysis: any) {
  ...
  if (analysis.moistureDeviation !== null) {
    ...
    if (Math.abs(dev) > TOLERANCE) {
      recommendations.push({
        ...
        severity: Math.abs(dev) > HIGH_TOLERANCE ? "warning" as const : "info" as const,
